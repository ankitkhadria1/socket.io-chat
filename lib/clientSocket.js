		...
		_createClass(ClientSocket, [{
			...
			value: function onAuthenticate(client, socket) {
				...
				client.emit(client.EVENTS.AUTHENTICATE, socket, data, function (error) {
					if (error) {
						return _this.emitError(socket, 'login', { message: error.message || error });
					}
					...
					_this.emitResult(socket, 'login', { user: socket.user });
				});
			}
		}, {
			key: 'onCreate',
			value: function onCreate(client, socket) {
				...
				client.create(data, socket.user).then(function (chat) {
					_this2.emitResult(socket, client.EVENTS.CREATE, { message: 'Chat created', data: chat.toJSON() });
					...
					client.__members.get(chat.get('members')).forEach(function (socket) {
						_this2.emitResult(socket, client.EVENTS.ENTER, { message: 'Added to chat', data: chat.toJSON() });
					});
				})['catch'](function (error) {
					return _this2.emitError(socket, client.EVENTS.CREATE, { message: error.message });
				});
			}
		}, {
			key: 'onLeave',
			value: function onLeave(client, socket) {
				...
				client.model('chat').findById(data.chatId).then(function (result) {
					...
					if (chat.systemMessages && chat.systemMessages.leaveMember) {
						client.newSystemMessage(chat, { whoLeaved: member }).then(function (message) {
							sockets.forEach(function (socket) {
								_this3.emitResult(socket, client.EVENTS.NEWMESSAGE, {
					...
					_this3.emitError(socket, client.EVENTS.LEAVE, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onAddMember',
			value: function onAddMember(client, socket) {
				...
				client.model('chat').findById(data.chatId).then(function (result) {
					...
					sockets.forEach(function (socket) {
						_this4.emitResult(socket, client.EVENTS.ADDMEMBER, {
					...
					_this4.emitError(socket, client.EVENTS.ADDMEMBER, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onRemoveMember',
			value: function onRemoveMember(client, socket) {
				...
				client.model('chat').findById(data.chatId).then(function (result) {
					...
					sockets.forEach(function (socket) {
						_this5.emitResult(socket, client.EVENTS.REMOVEMEMBER, {
					...
					_this5.emitError(socket, client.EVENTS.REMOVEMEMBER, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onNewMessage',
			value: function onNewMessage(client, socket) {
				...
				client.model('chat').findById(data.chatId).then(function (result) {
					...
					_this6.emitError(socket, client.EVENTS.NEWMESSAGE, { message: catchErrorMessage(error) });
				});
			}
		}, {
			...
			value: function onFindMessagesLast(client, socket) {
				...
				client.findLastMessages(data.chatId, socket.user, data.count).then(function (messages) {
					...
					_this7.emitError(socket, client.EVENTS.FINDMESSAGESLAST, { message: catchErrorMessage(error) });
				});
			}
		}, {
			...
			value: function onFindMessagesFrom(client, socket) {
				...
				client.findFromMessages(data.chatId, data.messageId, socket.user, data.count).then(function (messages) {
					...
					_this8.emitError(socket, client.EVENTS.FINDMESSAGESFROM, { message: catchErrorMessage(error) });
				});
			}
		}, {
			...
			value: function onFindMessagesAt(client, socket) {
				...
				client.findAtMessages(data.chatId, data.messageId, socket.user, data.count).then(function (messages) {
					_this9.emitResult(socket, client.EVENTS.FINDMESSAGESAT, { chatId: data.chatId, data: messages });
				})['catch'](function (error) {
					_this9.emitError(socket, client.EVENTS.FINDMESSAGESAT, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onFindChat',
			value: function onFindChat(client, socket) {
				...
				client.findChatById(socket.user, data.chatId).then(function (chat) {
					_this10.emitResult(socket, client.EVENTS.FINDCHAT, { data: chat });
				})['catch'](function (error) {
					_this10.emitError(socket, client.EVENTS.FINDCHAT, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onFindChats',
			value: function onFindChats(client, socket) {
				...
				client.findChats(socket.user, data.count).then(function (chats) {
					_this11.emitResult(socket, client.EVENTS.FINDCHATS, { data: chats });
				})['catch'](function (error) {
					_this11.emitError(socket, client.EVENTS.FINDCHATS, { message: catchErrorMessage(error) });
				});
			}
		}, {
			key: 'onChangeTitle',
				...
				client.model('chat').findById(data.chatId).then(function (chat) {
					...
					if (chat.systemMessages && chat.systemMessages.changeTitle) {
						client.newSystemMessage(chat, {
							...
							sockets.forEach(function (socket) {
								_this12.emitResult(socket, client.EVENTS.NEWMESSAGE, {
									message: 'New system message',
									data: message.toJSON(),
									chatId: chat.get('_id')
								});
					...
					sockets.forEach(function (socket) {
						_this12.emitResult(socket, client.EVENTS.CHANGETITLE, {
					...
					_this12.emitError(socket, client.EVENTS.CHANGETITLE, { message: catchErrorMessage(error) });
				});
			}
		}]);
