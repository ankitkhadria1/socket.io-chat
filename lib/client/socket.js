	...
	_createClass(ClientSocket, [{
		...
		value: function onLeave(client, socket) {
			...
			client.model('chat').findById(data.chatId).then(function (result) {
				if (!(chat = result)) {
					throw new _ClientError2['default']('Chat not found');
				}
...
client.on(client.EVENTS.CREATE).then(request.onCreate).then(client.create).then(response.onCreate)['catch'](next);
