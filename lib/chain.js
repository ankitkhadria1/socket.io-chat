...
var add = function add(list) {
	function push(callback) {
		if (!_underscore2['default'].isFunction(callback) && !(callback instanceof _middlewares2['default'])) {
			return (0, _debug.debug)('add chain middleware: callback is not a function', callback);
		}
...
var exec = function exec() {
	...
	return new Promise(function (resolve, reject) {
		...
		function next(error) {
			...
			if (chainItem) {
				index++;
				return chainItem instanceof _middlewares2['default'] ? chainItem.exec.apply(chainItem, data.concat(next)) : chainItem.apply(context, data.concat(next));
			}
