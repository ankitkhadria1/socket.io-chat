	...
	_createClass(BaseClient, [{
		...
		value: function initialize() {
			...
			this._io.on(IO_CONNECTION, function (socket) {
				...
				var wrapEventCallback = function wrapEventCallback(data) {
		...
		value: function use(path, middleware) {
			...
			if (_underscore2['default'].isArray(middleware)) middleware.forEach(function (_middleware) {
				_this2.__middleware[path].push(_middleware);
			});

			if (middleware.exec) if (_underscore2['default'].isArray(middleware.exec)) middleware.exec.forEach(function (_middleware) {
				_this2.__middleware[path].push(_middleware);
			});else this.__middleware[path].push(middleware.exec);
		...
		key: 'execMiddleware',
		value: function execMiddleware(path, data) {
		...
		key: 'invoke',
		value: function invoke(name, socket, data) {
			var _this3 = this;

			var promise = function promise(resolve, reject) {

				if (!(name in _this3.EVENTS)) {
					return reject();
				}
			};
		...
		value: function eventName(name) {
			...
			if (this._options[OPTION.EVENT_PREFIX]) {
				var _ret = (function () {
					var prefix = _this4._options[OPTION.EVENT_PREFIX];
		...
		value: function getChat(id) {
			...
			return new Promise(function (resolve, rejcet) {
				if (OPTION.CHAT_MEMORY in _this5._options) {} else {
					_this5.model.Chat.findById(id).then(function (chat) {
						resolve(chat);
					})['catch'](reject);
				}
			});
